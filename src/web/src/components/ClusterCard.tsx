import React from 'react';
import { Space, Typography, Divider, Image } from 'antd';
import { ClusterInfo } from '../types';
import { API_BASE_URL } from '../constants';

const { Text } = Typography;

interface ClusterCardProps {
  cluster: ClusterInfo;
}

const ClusterCard: React.FC<ClusterCardProps> = ({ cluster }) => {
  return (
    <div style={{ display: 'flex', alignItems: 'stretch', width: '100%', fontSize: 12 }}>
      {/* 左侧：统计信息卡片（固定宽度） */}
      <div style={{ flex: '0 0 320px', width: 320, height: '100%', display: 'flex', flexDirection: 'column', gap: 16 }}>
            {/* 基准LAB值 */}
            <div>
              <Divider orientation="left" style={{ fontSize: 12, margin: '8px 0' }}>基准LAB值</Divider>
              <Space orientation="vertical">
                <Text>
                  L: <Text strong>{cluster.lab_mean[0].toFixed(2)}</Text>
                  <Text type="secondary">（±{cluster.lab_std[0].toFixed(2)}）</Text>
                </Text>
                <Text>
                  a: <Text strong>{cluster.lab_mean[1].toFixed(2)}</Text>
                  <Text type="secondary">（±{cluster.lab_std[1].toFixed(2)}）</Text>
                </Text>
                <Text>
                  b: <Text strong>{cluster.lab_mean[2].toFixed(2)}</Text>
                  <Text type="secondary">（±{cluster.lab_std[2].toFixed(2)}）</Text>
                </Text>
              </Space>
            </div>

            {/* 与基准的误差 */}
            <div>
              <Divider orientation="left" style={{ fontSize: 12, margin: '8px 0' }}>与基准的误差</Divider>
              <Space orientation="vertical">
                <Text>
                  平均: <Text strong>{cluster.de2000_mean.toFixed(2)}</Text>
                </Text>
                <Text>
                  最大: <Text strong>{cluster.de2000_max.toFixed(2)}</Text>
                </Text>
                <Text>
                  标准差: <Text strong>{cluster.de2000_std.toFixed(2)}</Text>
                </Text>
              </Space>
            </div>

            {/* 样本间的误差 */}
            <div>
              <Divider orientation="left" style={{ fontSize: 12, margin: '8px 0' }}>样本间的误差</Divider>
              <Space orientation="vertical">
                <Text>
                  平均: <Text strong>{cluster.de2000_intra_mean.toFixed(2)}</Text>
                </Text>
                <Text>
                  最大: <Text strong>{cluster.de2000_intra_max.toFixed(2)}</Text>
                </Text>
              </Space>
            </div>
      </div>

      {/* 右侧：图片列表（占满剩余空间） */}
      <div style={{ flex: 1, minWidth: 0 }}>
        <Divider orientation="left" style={{ fontSize: 12, margin: '8px 0' }}>
          图片列表 ({cluster.image_paths.length} 张)
        </Divider>
        <div style={{ maxHeight: 500, overflowY: 'auto', padding: '8px 0' }}>
          <Space wrap size="small">
            {cluster.image_paths.map((path, idx) => {
              const imageUrl = `${API_BASE_URL}/api/image?path=${encodeURIComponent(path)}`;
              const fileName = path.split(/[/\\]/).pop() || '';

              return (
                <div
                  key={idx}
                  style={{
                    display: 'inline-block',
                    textAlign: 'center',
                    margin: 4,
                    verticalAlign: 'top',
                  }}
                >
                  <Image
                    src={imageUrl}
                    alt={fileName}
                    width={100}
                    height={100}
                    style={{
                      objectFit: 'cover',
                      border: '1px solid #d9d9d9',
                      borderRadius: 0,
                    }}
                    preview={{
                      mask: '预览',
                    }}
                    fallback="data:image/png;base64,"
                    onError={() => {
                      console.error('图片加载失败:', path);
                    }}
                  />
                  <div
                    style={{
                      marginTop: 4,
                      maxWidth: 100,
                      overflow: 'hidden',
                      textOverflow: 'ellipsis',
                      whiteSpace: 'nowrap',
                    }}
                  >
                    <Text type="secondary" style={{ fontSize: 11 }} title={fileName}>
                      {fileName}
                    </Text>
                  </div>
                </div>
              );
            })}
          </Space>
        </div>
      </div>
    </div>
  );
};

export default ClusterCard;

